##The following questions are based on data on Oscar Award Nominations for Best
##Director between the years 1931 to 2010. To start you off,
...
nom_count_dict = {}
count=0
for x in nominated.values():
    for y in x:
        if y not in nom_count_dict:
            nom_count_dict[y]= count+1
        else:
            nom_count_dict[y]= nom_count_dict[y] + 1
...
for x in winners.values():
    for y in x:
        if y not in win_count_dict:
            win_count_dict[y]= count+1
        else:
            win_count_dict[y]= win_count_dict[y] + 1
    
print("win_count_dict = {}".format(win_count_dict))

#another method to 1B
win_count_dict = {}
...
for x in winners.values():
    for y in x:
        if y not in win_count_dict:
            ...
            win_count_dict[y]= win_count_dict[y] + 1
print(max(win_count_dict.values()))

for m,n in zip(win_count_dict.keys(), win_count_dict.values()):
    if n == max(win_count_dict.values()):
       most_win_director.append(m)
